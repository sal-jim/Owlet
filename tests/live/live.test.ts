...
d('live CLI (Twitter/X)', () => {
  ...
  it('follow/unfollow works (opt-in)', async () => {
    const followHandle = (process.env.OWLET_LIVE_FOLLOW_HANDLE ?? '').trim();
    if (!followHandle) {
      return;
    }
  ...
  it('bookmarks --folder-id works (opt-in)', async () => {
    const folderId = (process.env.OWLET_LIVE_BOOKMARK_FOLDER_ID ?? '').trim();
    if (!folderId) {
      return;
    }
    const bookmarks = await runBird(
  ...
  it('query-ids --fresh works (opt-in)', async () => {
    if (process.env.OWLET_LIVE_QUERY_IDS_FRESH !== '1') {
      return;
    }
  ...
  it('long-form tweet (article) extracts rich content (opt-in)', async () => {
    const longformTweetId = (process.env.OWLET_LIVE_LONGFORM_TWEET_ID ?? '').trim();
    if (!longformTweetId) {
